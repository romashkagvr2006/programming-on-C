...
/* The following macro definitions may be used to translate this file:
  ...
  STDIO - use standard Input/Output device
          (default is NOT used)
*/
#define STDIO 1 // use standard Input/Output device
...
FILEHANDLE handle;
   ...
   handle = std_file_open(name, openmode);

   if(handle > 0x8000) return(handle);
...
#endif
   if (fh > 0x8000) {
   return(std_file_write(fh, buf, len, mode));
// return (-1);
   }
...
U32 cnt;
   ...
   if (fh == STDIN) {
      ...
      for (cnt = 0; cnt < len; cnt++)
	    {
        ch = getkey ();
        if(ch == 0x1B) return ( (len - cnt) | 0x80000000); // ESC --> End of file
		*buf++ =  ch;
        }
      return (0);
   }
#endif
   if (fh > 0x8000) {
      return(std_file_read(fh, buf, len, mode));
//    return (-1);
   }
